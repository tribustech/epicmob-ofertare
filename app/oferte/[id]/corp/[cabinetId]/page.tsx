...
export default async function CorpPage({ params }: { params: Promise<{ id: string; cabinetId: string }> }) {
  ...
  if (!cab || cab.quoteId !== id) notFound();
  ...
  const {
    quote, materials, edgeBands, settings, hardwareItems, snapshot,
    ...
    <div className="xl:relative xl:left-1/2 xl:w-[min(1800px,calc(100vw-3rem))] xl:-translate-x-1/2">
      <div className="space-y-5">
        ...
        <CabinetEditorForm
          ...
          ralColors={ralColors}
          quoteHandle={{
            type: quote.handleType, itemId: quote.handleItemId,
            label: HANDLE_TYPE_OPTIONS.find((o) => o.value === quote.handleType)?.label ?? quote.handleType,
          }}
