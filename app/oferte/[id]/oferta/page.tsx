...
export default async function OfertaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const clientName = ctx?.project?.client?.name ?? quoteRow.clientName ?? null;
  const clientContact = ctx?.project?.client?.phone ?? ctx?.project?.client?.email ?? quoteRow.clientContact ?? null;

  const basis = await getQuoteBasis(quoteRow);
  if (basis.kind === 'MISSING') {
    return (
      <p className="text-sm">
        Proiectul e într-o stare înghețată dar nu are un calcul salvat — <Link href={`/oferte/${id}`} className="underline">înapoi la proiect</Link> și comută starea pentru a genera un calcul.
      </p>
    );
  }
  ...
  const freeLines = JSON.parse(quoteRow.freeLinesJson) as { name: string; amount: number }[];
      ...
      <div className="sheet">
        ...
        <div className="band">
          <div className="party">
            ...
            <div className="name">{clientName || 'Beneficiar'}</div>
            {clientContact && <div className="sub">{clientContact}</div>}
          </div>
          <div className="party">
            <p className="eyebrow">Proiect</p>
            <div className="name">{quoteRow.name}</div>
        ...
        {quoteRow.observatii && (
          <section className="section">
            ...
            <p className="obs">{quoteRow.observatii}</p>
          </section>
        )}

        <div className="sign">
          <div className="slot"><div className="role">Furnizor</div><div className="who">EpicMob SRL · CUI {CUI}</div></div>
          <div className="slot"><div className="role">Beneficiar</div><div className="who">{clientName || '—'}</div></div>
        </div>
